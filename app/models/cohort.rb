class Cohort
  ...
  def initialize(start_date, end_date)
    ...
    @@first_registration_date = PatientProgram.where(["program_id = ? AND voided = 0",1]
  ...
  def patients_reinitiated_on_art_ever
    Observation.joins([:encounter]).where(["concept_id = ? AND value_coded IN (?) AND encounter.voided = 0 \
        AND DATE_FORMAT(obs_datetime, '%Y-%m-%d') <= ?", ConceptName.find_by_name("EVER RECEIVED ART").concept_id,
        ConceptName.where(["name = 'YES'"]).collect{|c| c.concept_id},
  ...
  def total_registered(start_date = @start_date, end_date = @end_date)
    ...
    #end_date = @end_date
    on_art_concept_name = ConceptName.where(["name =?", 'On antiretrovirals'])
  ...
  def patients_reinitiated_on_art(start_date = @start_date, end_date = @end_date)
    ...
    defaulted = ConceptName.where(["name =?", "DEFAULTED"])
    defaulted_state = ProgramWorkflowState.where(["concept_id IN (?)",
        defaulted.map{|c|c.concept_id}]).first.program_workflow_state_id

		treatment_stopped = ConceptName.where(["name =?", "TREATMENT STOPPED"])
    treatment_stopped_state = ProgramWorkflowState.where(["concept_id IN (?)",
        treatment_stopped.map{|c|c.concept_id}]).first.program_workflow_state_id
      ...
      if ob.concept_id.to_s == date_art_last_taken_concept.to_s
        ...
        state = PatientState.where(["patient_program_id = #{patient_program_id} AND end_date IS NOT NULL"]).order("date_created ASC").last rescue 0
        if !state.blank?
  ...
  def total_alive_and_on_art
    on_art_concept_name = ConceptName.where(["name =?", "On antiretrovirals"])
    state = ProgramWorkflowState.where(["concept_id IN (?)", on_art_concept_name.map{|c|c.concept_id}]).first.program_workflow_state_id
  ...
  def outcomes_total(outcome)
    on_art_concept_name = ConceptName.where(["name =?", outcome])
    state = ProgramWorkflowState.where(["concept_id IN (?)", on_art_concept_name.map{|c|c.concept_id}]).first.program_workflow_state_id
  ...
  def side_effect_patients(start_date = @start_date, end_date = @end_date)
    ...
    Observation.where(["encounter_id IN (#{encounter_ids.join(',')}) AND concept_id = ?  AND
      value_coded IN (#{side_effect_concept_ids.join(',')})",concept_id]).group("person_id").length
  end
