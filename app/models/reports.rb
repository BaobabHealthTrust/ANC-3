...
class Reports
  ...
  def initialize(start_date, end_date, start_age, end_age, type, today=Date.today)
    ...
    if type == 'cohort'
      ...
      @cohort_patients = Encounter.joins([:observations]).where(['encounter_type = ? AND concept_id = ? AND
          encounter_datetime >= ? AND encounter_datetime <= ? AND encounter.voided = 0',
          ANC_VISIT_TYPE_ENCOUNTER.id, TYPE_OF_VISIT_CONCEPT.concept_id,
          @start_date, @end_date]).group([:patient_id]).select(['patient_id']).collect { |e| e.patient_id }.uniq
    end
  ...
  def registrations(start_dt, end_dt)
    ...
    Encounter.joins(['INNER JOIN obs ON obs.person_id = encounter.patient_id']).where(['encounter_type = ? AND
      obs.concept_id = ? AND DATE(encounter_datetime) >= ? AND DATE(encounter_datetime) <= ? AND encounter.voided = 0',
        CURRENT_PREGNANCY_ENCOUNTER.id, LMP_CONCEPT.concept_id, start_dt.to_date,end_dt.to_date
      ]).select(['MAX(value_datetime) lmp, patient_id']).group([:patient_id]).collect { |e| e.patient_id }.uniq
  ...
  def pre_eclampsia_2
    ...
    Encounter.joins([:observations]).where(["concept_id = ? AND value_coded = ? AND DATE(encounter_datetime) BETWEEN (#{@lmp}) AND (?)" +
  ...
  def pre_eclampsia_1
    ...
    @cases1 = Encounter.joins([:observations]).where(["concept_id = ? AND value_coded IN (?) AND DATE(encounter_datetime)
      BETWEEN (#{@lmp}) AND (?)" +
          "AND encounter.patient_id IN (?)",
  ...
  def ttv__total_previous_doses_2(tag = 2)
    ...
    Order.joins([[:drug_order => :drug], :encounter]).where(["drug.name LIKE ? AND (DATE(encounter_datetime) >= ? " +
          "AND DATE(encounter_datetime) <= ?) AND encounter.patient_id IN (?) AND orders.voided = 0", "%TTV%",
        @lmp, ((@start_date.to_date + @pregnant_range) - 1.day), @cohort_patients]).group([:patient_id]).select(["encounter.patient_id, count(*) encounter_id"]).collect { |o|
  ...
  def fansida__sp
    ...
    Order.joins([[:drug_order => :drug], :encounter]).where(["drug.name = ?  AND (DATE(encounter_datetime) >= #{@lmp}" +
        ...
        ((@start_date.to_date + @pregnant_range) - 1.day), @cohort_patients]).select(["encounter.patient_id, DATE(encounter_datetime) datetime, drug.name instructions"]).each { |o|
  ...
  def fansida__sp___number_of_tablets_given_3
    ...
    p = Order.joins([[:drug_order => :drug], :encounter]).where(["(drug.name = ? OR drug.name = ?) " +
          "AND DATE(encounter_datetime) <= ? AND encounter.patient_id IN (?)",
        "Sulphadoxine and Pyrimenthane (25mg tablet)", "SP (3 tablets)",
        ((@start_date.to_date + @pregnant_range) - 1.day), @cohort_patients]).group([:patient_id]).select(["encounter.patient_id, count(*) encounter_id, drug.name instructions"]).collect { |o|
...
=end
  def syphilis_result_pos
    ...
    Encounter.joins([:observations]).where(["concept_id = ? AND (value_coded = ? OR value_text = ?) AND (DATE(encounter_datetime) >= #{@lmp} " +
          "AND DATE(encounter_datetime) <= ?) AND encounter.patient_id IN (?)",
        ConceptName.find_by_name("Syphilis Test Result").concept_id,
        ConceptName.find_by_name("Positive").concept_id, "Positive",
        ((@start_date.to_date + @pregnant_range) - 1.day), @cohort_patients]).select(["DISTINCT patient_id"]).collect { |e| e.patient_id }
  end
  ...
  def final_visit_new_negative
    hiv_test_concept  = ConceptName.find_by_name("HIV Status")
    hiv_neg_concept   = ConceptName.find_by_name("Negative")
    ...
    select = Encounter.find_by_sql([querystmnt, HIV_TEST_DATE_CONCEPT.concept_id, hiv_test_concept.concept_id,
        hiv_neg_concept.concept_id, @cohort_patients, hiv_test_concept.concept_id, 
        ((@start_date.to_date + @pregnant_range) - 1.day), ((@start_date.to_date + @pregnant_range) - 1.day)])
      .map(&:patient_id)
  ...
  def final_visit_hiv_not_done
    ...
    select = Encounter.joins([:observations]).where(["concept_id = ? AND (value_coded = ? OR value_text = ?) AND (DATE(encounter_datetime) >= #{@lmp} " +
          "AND DATE(encounter_datetime) <= ?) AND encounter.patient_id IN (?)",
        ConceptName.find_by_name("HIV status").concept_id,
        ConceptName.find_by_name("Not done").concept_id, "Not Done",
        ((@start_date.to_date + @pregnant_range) - 1.day), first_visit_patient_ids]).group(["patient_id"]).select(["patient_id, MAX(encounter_datetime) encounter_datetime, (obs_id + 1) form_id"]).collect { |e| e.patient_id }
...
=end
  ...
  def first_visit_hiv_not_done
    ...
    select = Encounter.joins([:observations]).where(["concept_id = ? AND (value_coded = ? OR value_text = ?) AND (DATE(encounter_datetime) >=
        ...
        @today.to_date, @monthly_patients]).group(["patient_id"]).select(["patient_id, MAX(encounter_datetime) encounter_datetime, (obs_id + 1) form_id"]).collect { |e| e.patient_id }
  ...
  def hiv_test_result_neg
    ...
    querystmnt  = "SELECT e.patient_id, e.encounter_datetime AS date, "
    querystmnt += "(SELECT value_datetime FROM obs WHERE encounter_id = e.encounter_id "
    querystmnt += "AND obs.concept_id = ?) AS test_date FROM encounter e "
    querystmnt += "INNER JOIN obs o ON o.encounter_id = e.encounter_id AND e.voided = 0 "
    querystmnt += "WHERE o.concept_id = ? AND ((o.value_coded = ?) "
    querystmnt += "OR (o.value_text = 'Negative')) AND e.patient_id IN (?) "
    ...
    select = Encounter.find_by_sql([querystmnt, HIV_TEST_DATE_CONCEPT.concept_id, hiv_test_concept.concept_id,
      hiv_neg_concept.concept_id, @cohort_patients, ((@start_date.to_date + @pregnant_range) - 1.day), 
      ((@start_date.to_date + @pregnant_range) - 1.day)]).map(&:patient_id)
  ...
  def hiv_test_result_unk
    ...
    select = Encounter.joins([:observations]).where(["concept_id = ? AND (value_coded = ? OR value_text = ?) AND (DATE(encounter_datetime) >= #{@lmp} " +
        ...
        ConceptName.find_by_name("HIV status").concept_id,
        ConceptName.find_by_name("Not done").concept_id, "Not Done",
        ((@start_date.to_date + @pregnant_range) - 1.day), @cohort_patients]).group(["patient_id"]).select(["patient_id, MAX(encounter_datetime) encounter_datetime, (obs_id + 1) form_id"]).collect { |e| e.patient_id }
...
=end
  ...
  def on_art_in_bart
    ...
    patient_ids = PatientIdentifier.where(["identifier_type = ? AND patient_id IN (?)", national_id,
        @first_visit_positive_patients]).select(['identifier, identifier_type']).collect { |ident|
  ...
  def on_art_in_bart_first_visit
    ...
    patient_ids = PatientIdentifier.where(['identifier_type = ? AND patient_id IN (?)',
        national_id, @first_visit_positive_patients]
  ...
  def on_art_in_bart_final_visit
    national_id = PatientIdentifierType.find_by_name("National id").id
    patient_ids = PatientIdentifier.where(["identifier_type = ? AND patient_id IN (?)", national_id,
        @final_visit_positive_patients]).select(['identifier, identifier_type']).collect { |ident|
      ident.identifier }.join(",")
    id_visit_map = []
    patient_ids.split(",").each do |id|
      next if id.nil?
  ...
  def total_monthly_anc_visits
    ...
    Encounter.joins(['INNER JOIN obs ON obs.person_id = encounter.patient_id']).where(['DATE(encounter_datetime) >= ?
  ...
  def women_received_sp_1
    Order.joins([[:drug_order => :drug], :encounter]).where(["(drug.name = ? OR drug.name = ?) " +
        ...
        @monthly_start_date.to_date.beginning_of_month, 
        @monthly_end_date.to_date.end_of_month, 
        @new_monthly_visits]).group([:patient_id]).select(["encounter.patient_id,
  ...
  def women_received_ttv_doses
    ...
    rec_ttv = Order.joins([[:drug_order => :drug], :encounter]).where(["drug.name LIKE ? AND encounter.patient_id IN (?)
        AND orders.voided = 0", "%TTV%", @new_monthly_visits]).group([:patient_id]).select(["encounter.patient_id, count(*) encounter_id"]).collect { |o|
      o.patient_id
  ...
  def women_received_iron
    fefol = {}
    results = []
    Order.joins([[:drug_order => :drug], :encounter]).where(["drug.name = ? AND encounter.patient_id IN (?)",
        "Fefol (1 tablet)",@new_monthly_visits]).group([:patient_id]).select(["encounter.patient_id, count(*) datetime, drug.name instructions," +
  ...
  def women_received_albendazole
    ...
    data = Order.joins([[:drug_order => :drug], :encounter]).where(["drug.name REGEXP ? AND "+
          "encounter.patient_id IN (?)", "Albendazole",
        @new_monthly_visits]).group([:patient_id]).select(["encounter.patient_id, count(*) encounter_id, "+
  ...
  def women_tested_hiv_positive
    ...
    select = Encounter.find_by_sql(["SELECT e.patient_id
      ...
      AND e.patient_id IN (?)",
      hiv_status_concept.concept_id, hiv_pos_concept.concept_id, @new_monthly_visits
      ]).map(&:patient_id)
  ...
  def women_previously_tested_hiv_positive
    hiv_pos_concept = ConceptName.find_by_name("positive")
    prev_hiv_status_concept = ConceptName.find_by_name("Previous HIV Test Results")
