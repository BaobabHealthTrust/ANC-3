...
ORDER BY clinic ASC"])
  ...
  def self.set_appointments(date = Date.today,identifier_type = 'Filing number')
    ...
    records = Observation.joins("INNER JOIN person p 
      ...
      FROM patient_identifier_type pi WHERE pi.name = '#{identifier_type}')").where(
      ["obs.concept_id=? AND value_datetime >= ? AND value_datetime <=?",
        concept_id,date.strftime('%Y-%m-%d 00:00:00'),date.strftime('%Y-%m-%d 23:59:59')
      ]).order("obs.obs_datetime DESC").select("obs.obs_id obs_id,obs.person_id patient_id,n.given_name first_name,n.family_name last_name,
