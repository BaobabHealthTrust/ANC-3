class Property
  ...
  def self.clinic_appointment_limit(end_date = nil)
    ...
    Observation.joins("INNER JOIN encounter e USING(encounter_id)").where(["encounter_type = ? AND value_datetime IS NOT NULL
    AND (DATE(value_datetime) >= ? AND DATE(value_datetime) <= ?)",
    encounter_type.id,start_date,end_date]).order("value_datetime DESC").map do | obs |
