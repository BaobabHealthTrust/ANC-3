...
# ==========================================================
  ...
  def reassign_national_identifier
    ...
    patient_ids = PatientIdentifier.where(["patient_id = ?", params[:person_id]])
    ...
    unless patient_ids.blank?
      patient_ids.each do |pat|
        ...
        if create_from_remote && pat["identifier_type"] == 2
          ...
          old_national_id = PatientIdentifier.where(["patient_id = ? and identifier_type = 3", patient.id]).first rescue nil
          if !old_national_id.blank?
            old_national_id.update_attributes(:identifier => new_npid)
          else
            legacy = PatientIdentifier.where(["patient_id = ? and identifier_type = 2", patient.id]).first rescue nil
  ...
  def static_nationalities
    ...
    render plain: "<li></li><li " + nationalities.map{|nationality| "value=\"#{nationality}\">#{nationality}" }.join("</li><li ") + "</li>"
