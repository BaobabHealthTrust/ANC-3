class GenericPeopleController < ApplicationController
  ...
  def search_from_dde
    ...
    @current_hiv_program_state = PatientProgram.joins(:location).where(["program_id = ? AND patient_id = ? AND location.location_id = ?",
        Program.find_by_concept_id(Concept.find_by_name('HIV PROGRAM').id).id,@person.patient,
        Location.current_health_center]).first.patient_states.last.program_workflow_state.concept.fullname rescue ''
  ...
  def traditional_authority
    ...
    traditional_authorities = TraditionalAuthority.where(traditional_authority_conditions).order("name") rescue []
  ...
  def traditional_authority_for
    ...
    render plain: (traditional_authorities + ["Other"]).join('|') and return
  end
  ...
  def districts_for
    ...
    if region_id.blank?
      nationalities = []
      File.open(Rails.root.to_s + "/public/data/nationalities.txt", "r").each{ |nat|
        nationalities << nat
      }
    ...
    render plain: (districts + ["Other"]).join('|')  and return
  end
  ...
  def village
    ...
    villages = Village.where(village_conditions).order("name") rescue []
...
=end
  ...
  def art_info_for_remote(national_id)
    ...
    if PatientService.art_patient?(patient)
      ...
      first_encounter_date = patient.encounters.where(['encounter_type IN (?)',clinic_encounter_ids]).order("encounter_datetime").first.encounter_datetime.strftime("%d-%b-%Y") rescue 'Uknown'

      last_encounter_date = patient.encounters.where(['encounter_type IN (?)',clinic_encounter_ids]).order("encounter_datetime DESC").first.encounter_datetime.strftime("%d-%b-%Y") rescue 'Uknown'
