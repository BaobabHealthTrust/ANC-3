class GenericUserController < ApplicationController
  ...
  def voided_list
    ...
    @user_pages, @users = User.where(["voided=1"]).paginate(per_page: 50)#User.paginate#paginate(:users, :per_page => 50,:conditions =>["voided=1"])
    render :view => 'list'
  end
  ...
  def activities
    ...
    role_privileges = RolePrivilege.where(["role IN (?)", user_roles])
