class ConfigController < ApplicationController
  ...
  def concept
  	@cnames=ConceptName.order("date_created DESC").collect{|cc| [cc.name,cc.concept_id]}
  	@cclasses=ConceptClass.order("date_created DESC").collect{|cc| [cc.name,cc.concept_class_id]}
    @cdatatypes=ConceptDatatype.order("date_created DESC").collect{|cdt| [cdt.name, cdt.concept_datatype_id] }
  	if request.post?
