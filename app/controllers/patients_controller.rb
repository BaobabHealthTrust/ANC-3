class PatientsController < ApplicationController
  ...
  def show      
    ...
    @encounters = @patient.encounters.where(["encounter_datetime >= ? AND encounter_datetime <= ?",
        @current_range[0]["START"], @current_range[0]["END"]]) rescue []

    @all_encounters = @patient.encounters.all rescue []

    @encounter_names = @patient.encounters.where(["encounter_datetime >= ? AND encounter_datetime <= ?",
    ...
    @current_encounter_names = @patient.encounters.where(["DATE_FORMAT(encounter_datetime, '%Y-%m-%d') = ?", (session[:datetime] ? session[:datetime].to_date : Date.today)]).collect{|e|
  ...
  def next_available_arv_number
    ...
    render plain: next_available_arv_number.gsub(Location.current_arv_code,'').strip rescue nil
  end
  ...
  def tab_visit_summary
    session_date = session[:datetime].to_date rescue Date.today
    @data = []
    @encounters = @patient.encounters.where([" DATE(encounter_datetime) <= ? ",session_date]).order("encounter_datetime DESC") rescue []

    @external_encounters = []

    if !File.exists?("#{Rails.root.to_s}/config/dde_connection.yml")
      @external_encounters = Bart2Connection::PatientIdentifier.search_by_identifier(@anc_patient.national_id
      ).patient.encounters.where([" DATE(encounter_datetime) <= ? ", session_date]) rescue [] if @anc_patient.hiv_status.downcase == "positive"
    else
  ...
  def tab_obstetric_history
    ...
    @all_enc = @patient.encounters.all.collect{|e| e.encounter_id}

    encs = @patient.encounters.where(["encounter_type = ?", EncounterType.find_by_name("OBSTETRIC HISTORY").id])

    @encs = encs.length

    if @encs > 0
      @deliveries = Observation.where(["person_id = ? AND encounter_id IN (?) AND concept_id = ?", @patient.id, @all_enc,
          ConceptName.find_by_name('PARITY').concept_id]).last.answer_string.to_i rescue nil

      @deliveries = @deliveries + (@range.length > 0 ? @range.length - 1 : @range.length) if !@deliveries.nil?

      @gravida = Observation.where(["person_id = ? AND encounter_id IN (?) AND concept_id = ?", @patient.id, @all_enc,
          ConceptName.find_by_name('GRAVIDA').concept_id]).last.answer_string.to_i rescue nil

      @abortions = Observation.where(["person_id = ? AND encounter_id IN (?) AND concept_id = ?", @patient.id, @all_enc,
          ConceptName.find_by_name('NUMBER OF ABORTIONS').concept_id]).last.answer_string.to_i rescue nil

      @stillbirths = Observation.where(["person_id = ? AND encounter_id IN (?) AND concept_id = ?", @patient.id, @all_enc,
          ConceptName.find_by_name('STILL BIRTH').concept_id]).last.answer_string.upcase.squish rescue nil
      ...
      @csections = Observation.where(["person_id = ? AND encounter_id IN (?) AND (concept_id = ? AND (value_coded = ? OR value_text = 'Yes'))", @patient.id,
      ...
      @vacuum = Observation.where(["person_id = ? AND encounter_id IN (?) AND (value_coded = ? OR value_text = 'Yes')", @patient.id,
          encs.collect{|e| e.encounter_id},
          ConceptName.find_by_name('Vacuum extraction delivery').concept_id]).length rescue nil

      @episiotomy = Observation.where(["person_id = ? AND encounter_id IN (?) AND concept_id = ?", @patient.id, @all_enc,
          ConceptName.find_by_name('EPISIOTOMY').concept_id]).last.answer_string.upcase.squish rescue nil

      @haemorrhage = Observation.where(["person_id = ? AND encounter_id IN (?) AND concept_id = ?", @patient.id, @all_enc,
          ConceptName.find_by_name('HEMORRHAGE').concept_id]).last.answer_string.upcase.squish rescue nil

      @preeclampsia = Observation.where(["person_id = ? AND encounter_id IN (?) AND concept_id = ?", @patient.id, @all_enc,
          ConceptName.find_by_name('PRE-ECLAMPSIA').concept_id]).last.answer_string.upcase.squish rescue nil

      @eclampsia = Observation.where(["person_id = ? AND encounter_id IN (?) AND concept_id = ?", @patient.id, @all_enc,
          ConceptName.find_by_name('ECLAMPSIA').concept_id]).last.answer_string.upcase.squish rescue nil
    end
  ...
  def tab_medical_history
    ...
    @asthma = Observation.where(["person_id = ? AND encounter_id IN (?) AND concept_id = ?",
        @patient.id, Encounter.where(["patient_id = ?", @patient.id]).collect{|e| e.encounter_id},
        ConceptName.find_by_name('ASTHMA').concept_id]).last.answer_string.upcase.squish rescue nil

    @hyper = Observation.where(["person_id = ? AND encounter_id IN (?) AND concept_id = ?",
        @patient.id, Encounter.where(["patient_id = ?", @patient.id]).collect{|e| e.encounter_id},
        ConceptName.find_by_name('HYPERTENSION').concept_id]).last.answer_string.upcase.squish rescue nil

    @diabetes = Observation.where(["person_id = ? AND encounter_id IN (?) AND concept_id = ?",
        @patient.id, Encounter.where(["patient_id = ?", @patient.id]).collect{|e| e.encounter_id},
        ConceptName.find_by_name('DIABETES').concept_id]).last.answer_string.upcase.squish rescue nil

    @epilepsy = Observation.where(["person_id = ? AND encounter_id IN (?) AND concept_id = ?",
        @patient.id, Encounter.where(["patient_id = ?", @patient.id]).collect{|e| e.encounter_id},
        ConceptName.find_by_name('EPILEPSY').concept_id]).last.answer_string.upcase.squish rescue nil

    @renal = Observation.where(["person_id = ? AND encounter_id IN (?) AND concept_id = ?",
        @patient.id, Encounter.where(["patient_id = ?", @patient.id]).collect{|e| e.encounter_id},
        ConceptName.find_by_name('RENAL DISEASE').concept_id]).last.answer_string.upcase.squish rescue nil

    @fistula = Observation.where(["person_id = ? AND encounter_id IN (?) AND concept_id = ?",
        @patient.id, Encounter.where(["patient_id = ?", @patient.id]).collect{|e| e.encounter_id},
        ConceptName.find_by_name('FISTULA REPAIR').concept_id]).last.answer_string.upcase.squish rescue nil

    @deform = Observation.where(["person_id = ? AND encounter_id IN (?) AND concept_id = ?",
        @patient.id, Encounter.where(["patient_id = ?", @patient.id]).collect{|e| e.encounter_id},
        ConceptName.find_by_name('SPINE OR LEG DEFORM').concept_id]).last.answer_string.upcase.squish rescue nil
    ...
    @surgicals = Observation.where(["person_id = ? AND encounter_id IN (?) AND concept_id = ?",
      ...
      "#{o.answer_string.squish} (#{Observation.where(["obs_id =?", o.id + 1]).where(["concept_id = ?",
      ConceptName.find_by_name("Date Received").concept_id]).last.value_datetime.to_date.strftime('%d-%b-%Y') rescue "Unknown" })"} rescue []

    @blood_transfusion = Observation.where(["person_id = ? AND encounter_id IN (?) AND concept_id = ?",
        @patient.id, Encounter.where(["patient_id = ?", @patient.id]).collect{|e| e.encounter_id},
        ConceptName.find_by_name('BLOOD TRANSFUSION').concept_id]).last.answer_string.upcase.squish rescue nil

    @blood_transfusion = Observation.where(["person_id = ? AND encounter_id IN (?) AND concept_id = ?",
        @patient.id, Encounter.where(["patient_id = ?", @patient.id]).collect{|e| e.encounter_id},
        ConceptName.find_by_name('BLOOD TRANSFUSION').concept_id]).last.answer_string.upcase.squish rescue nil

    @sti = Observation.where(["person_id = ? AND encounter_id IN (?) AND concept_id = ?",
        @patient.id, Encounter.where(["patient_id = ?", @patient.id]).collect{|e| e.encounter_id},
        ConceptName.find_by_name('Sexually transmitted infection').concept_id]).last.answer_string.upcase.squish rescue nil
  ...
  def tab_examinations_management
    ...
    @multiple = Observation.where(["person_id = ? AND encounter_id IN (?) AND concept_id = ?",
        @patient.id, Encounter.all.collect{|e| e.encounter_id},
        ConceptName.find_by_name('EVER HAD A MULTIPLE PREGNANCY?').concept_id]).last.answer_string rescue nil

    @who = Observation.where(["person_id = ? AND encounter_id IN (?) AND concept_id = ?",
        @patient.id, Encounter.all.collect{|e| e.encounter_id},
        ConceptName.find_by_name('WHO CLINICAL STAGE').concept_id]).last.answer_string.to_i rescue nil
  ...
  def tab_lab_results
    ...
    @multiple = Observation.where(["person_id = ? AND encounter_id IN (?) AND concept_id = ?",
        @patient.id, Encounter.where(["encounter_type = ?", EncounterType.find_by_name("CURRENT PREGNANCY").id]).collect{|e| e.encounter_id},
        ConceptName.find_by_name('Multiple Gestation').concept_id]).last.answer_string.squish rescue nil

    @who = ConceptName.find_by_concept_name_id(Observation.where(["person_id = ? AND concept_id = ?", @patient.id,
          ConceptName.find_by_name("WHO Stage").concept_id]).last.value_coded_name_id).name rescue nil
  ...
  def pregnancy_history
    ...
    @gravida = Observation.where(["person_id = ? AND encounter_id IN (?) AND concept_id = ?", @patient.id,
        Encounter.all.collect{|e| e.encounter_id},
        ConceptName.find_by_name('GRAVIDA').concept_id]).last.answer_string.to_i rescue nil
  ...
  def outcome
    @program_id = PatientProgram.where(["patient_id =?", @patient.id]).where(["program_id = ?", Program.find_by_name("ANC PROGRAM").id]).last.patient_program_id rescue nil
  end

  def current_visit
    @enc_types =  EncounterType.where(["name in ('ART_FOLLOWUP', 'PREGNANCY STATUS', 'OBSERVATIONS', 'VITALS', 'TREATMENT', 'LAB RESULTS', " +
          "'DIAGNOSIS', 'APPOINTMENT', 'UPDATE OUTCOME')"]).collect{|t| t.id}

    @encounters = @patient.encounters.where(["encounter_type IN (?) AND " +
    ...
    @all_encounters = @patient.encounters.where(["encounter_type IN (?)",
        @enc_types]).collect{|e|
      e.type.name
    }.join(", ")
    ...
    @preg_encounters = @patient.encounters.where(["encounter_datetime >= ? AND encounter_datetime <= ?",
        @current_range[0]["START"], @current_range[0]["END"]]) rescue []
  ...
  def number_of_booked_patients
    ...
    count = Observation.joins("INNER JOIN encounter e USING(encounter_id)").where(["concept_id = ? AND encounter_type = ? AND value_datetime >= ? AND value_datetime <= ?",
        concept_id,encounter_type.id,date.strftime('%Y-%m-%d 00:00:00'),date.strftime('%Y-%m-%d 23:59:59')]).group("value_datetime").count
    count = count.values unless count.blank?
    count = '0' if count.blank?
    render plain: (count.first.to_i > 0 ? {params[:date] => count}.to_json : 0)
  end
  ...
  def tab_social_history
    ...
    @alcohol = Observation.where(["person_id = ? AND encounter_id IN (?) AND concept_id = ?",
        @patient.id, Encounter.where(["patient_id = ?", @patient.id]).collect{|e| e.encounter_id},
        ConceptName.find_by_name('Patient currently consumes alcohol').concept_id]).last.answer_string.squish  rescue nil

    @smokes = Observation.where(["person_id = ? AND encounter_id IN (?) AND concept_id = ?",
        @patient.id, Encounter.where(["patient_id = ?", @patient.id]).collect{|e| e.encounter_id},
        ConceptName.find_by_name('Patient currently smokes').concept_id]).last.answer_string.squish rescue nil

    @nutrition = Observation.where(["person_id = ? AND encounter_id IN (?) AND concept_id = ?",
        @patient.id, Encounter.where(["patient_id = ?", @patient.id]).collect{|e| e.encounter_id},
        ConceptName.find_by_name('Nutrition status').concept_id]).last.answer_string rescue nil

    @civil = Observation.where(["person_id = ? AND encounter_id IN (?) AND concept_id = ?",
        @patient.id, Encounter.where(["patient_id = ?", @patient.id]).collect{|e| e.encounter_id},
        ConceptName.find_by_name('Civil status').concept_id]).last.answer_string.titleize rescue nil

    @civil_other = (Observation.where(["person_id = ? AND encounter_id IN (?) AND concept_id = ?",
          @patient.id, Encounter.where(["patient_id = ?", @patient.id]).collect{|e| e.encounter_id},
          ConceptName.find_by_name('Other Civil Status Comment').concept_id]).last.answer_string rescue nil) if @civil == "Other"

    @religion = Observation.where(["person_id = ? AND encounter_id IN (?) AND concept_id = ?",
        @patient.id, Encounter.where(["patient_id = ?", @patient.id]).collect{|e| e.encounter_id},
        ConceptName.find_by_name('Religion').concept_id]).last.answer_string.titleize rescue nil

    @religion_other = (Observation.where(["person_id = ? AND encounter_id IN (?) AND concept_id = ?",
          @patient.id, Encounter.where(["patient_id = ? AND encounter_type = ?",
              @patient.id, EncounterType.find_by_name("SOCIAL HISTORY").id]).collect{|e| e.encounter_id},
          ConceptName.find_by_name('Other').concept_id]).last.answer_string rescue nil) if @religion == "Other"
  ...
  def print_exam_label
    ...
    @lab_encounters = @patient.encounters.where(["encounter_type IN (?)",
  ...
  def visit_type
    @program_id = PatientProgram.where(["patient_id =?", @patient.id]).where(["program_id = ?",
        Program.find_by_name("ANC PROGRAM").id]).last.patient_program_id rescue nil
  ...
  def confirm
    ...
    @current_pregnancy = @patient.encounters.where(["encounter_type = ? AND voided = 0 AND DATE(encounter_datetime) BETWEEN ? AND ?",
        EncounterType.find_by_name("CURRENT PREGNANCY"), session_date.to_date - 10.months, session_date.to_date]).last
  ...
  def pdash_summary
    latest_encounters = Encounter.where(["patient_id = ? AND
      encounter_datetime >= ? AND encounter_datetime <= ?",params[:patient_id],
        ...
        params[:date].to_date.strftime('%Y-%m-%d 23:59:59')]).order("encounter_datetime ASC,date_created ASC")
  ...
  def surgical_history
    ...
    @csections = Observation.where(["person_id = ? AND (concept_id = ? AND (value_coded = ? OR value_text = 'Yes'))", @patient.id,
        ConceptName.find_by_name('Caesarean section').concept_id, ConceptName.find_by_name('Yes').concept_id]).length rescue nil
  end
  ...
  def search_all
    ...
    unless search_by_identifier
      patients = PatientIdentifier.where(["voided = 0 AND (identifier LIKE ?)",
                                                              "%#{search_str}%"]).limit(10).map{| p |p.patient}
    else
      given_name = search_str.split(' ')[0] rescue ''
      family_name = search_str.split(' ')[1] rescue ''
      patients = PersonName.joins([:person => [:patient]]).where(["person.voided = 0 AND family_name LIKE ? AND given_name LIKE ?",
