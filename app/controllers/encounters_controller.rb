class EncountersController < ApplicationController
  ...
  def create
    ...
    @preg_encounters = @patient.encounters.where(["voided = 0 AND encounter_datetime >= ? AND encounter_datetime <= ?",
  ...
  def new
    ...
    if ! @last_vitals.blank?
      ...
      @vital["weight"] = Observation.where(['concept_id = ? AND voided = 0 AND person_id = ?',
          weight, params[:patient_id]]).last.to_s.split(':')[1] rescue ''

      @vital["height"] = Observation.where(['concept_id = ? AND voided = 0 AND person_id = ?',
          height, params[:patient_id]]).last.to_s.split(':')[1] rescue ''

      @vital["bmi"] = Observation.where(['concept_id = ? AND voided = 0 AND person_id = ?',
          bmi, params[:patient_id]]).last.to_s.split(':')[1] rescue ''
    else
      @first = 'true'
    end
  ...
  def diagnoses
    ...
    render plain: "<li>" + @results.join("</li><li>") + "</li>"
  ...
  def locations
    ...
    filter_list = params[:filter_list].split(/, */) rescue []    
    locations =  Location.where(["name LIKE ?", '%' + search_string + '%']).select("name")
    render plain: "<li>" + locations.map{|location| location.name }.join("</li><li>") + "</li>"
  end
  ...
  def probe_lmp
    ...
    result = Hash.new
    lmp =  Observation.where(["person_id = ? AND concept_id = ? AND voided = 0 AND obs_datetime > ?",
        patient_id, concept_id, 9.months.ago]).order(["obs_datetime"]).last.answer_string.to_date rescue nil if patient_id.present? and concept_id.present?
  ...
  def procedure_done
    ...
    render plain:  "<li>" + @procedure_done.join("</li><li>") + "</li>"
  end

  def yes_no_options    
   
    render plain:  (["Yes", "No"]).join('|')  and return
  end

  def hemorrhage_options

    render plain: (["No", "APH", "PPH"]).join('|')  and return
  end
