class GenericLabController < ApplicationController
  ...
  def id_identifiers(patient)
    ...
    identifier_types = PatientIdentifierType.where(["name IN (?)",identifier_type]).collect{| type |type.id }
    
    PatientIdentifier.where(["patient_id=? AND identifier_type IN (?)", patient.id,identifier_types]).collect{| i | i.identifier }
  end
