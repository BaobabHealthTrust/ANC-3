class GenericReportController < ApplicationController
  ...
  def weekly_report
    ...
    @diagnoses = ConceptName.joins("INNER JOIN obs ONconcept_name.concept_id = obs.value_coded AND obs.voided = 0").where(
      ["date_format(obs_datetime, '%Y-%m-%d') >= ? AND date_format(obs_datetime, '%Y-%m-%d') <= ?", @start_date, @end_date
      ]).group("name").select("concept_name.concept_id,concept_name.name,obs.value_coded,obs.obs_datetime,obs.voided")

    @patient = Person.joins("INNER JOIN obs ON person.person_id = obs.person_id AND obs.voided = 0").where(
      ["date_format(obs_datetime, '%Y-%m-%d') >= ? AND date_format(obs_datetime, '%Y-%m-%d') <= ?",@start_date, @end_date
  ...
  def disaggregated_diagnosis
    ...
    diagnoses = ConceptName.joins("INNER JOIN obs ON concept_name.concept_id = obs.value_coded AND obs.voided = 0").where(
      ["date_format(obs_datetime, '%Y-%m-%d') >= ? AND date_format(obs_datetime, '%Y-%m-%d') <= ?", @start_date, @end_date]
    ).group("name").select("concept_name.concept_id,concept_name.name,obs.value_coded,obs.obs_datetime,obs.voided")
    #getting list of all patients who were diagnosed within the set period-to avoid getting all patients

    @patient = Person.joins("INNER JOIN obs ON person.person_id = obs.person_id AND obs.voided = 0").where(
      ["date_format(obs_datetime, '%Y-%m-%d') >= ? AND date_format(obs_datetime, '%Y-%m-%d') <= ?",
        @start_date, @end_date]).select("person.gender,person.birthdate,person.birthdate_estimated,person.date_created,person.voided,obs.value_coded,obs.obs_datetime,obs.voided ")
  ...
  def referral
    ...
    @referrals = Observation.where(["concept_id = ? AND date_format(obs_datetime, '%Y-%m-%d') >= ? AND date_format(obs_datetime, '%Y-%m-%d') <= ?", 2227, @start_date, @end_date])

    @facilities = Observation.where(["concept_id = ?", 2227]).group("value_text")
  end
  ...
  def recorded_diagnosis
    ...
    @names = Observation.joins("INNER JOIN concept_name c ON obs.value_coded_name_id = c.concept_name_id").where(["obs.concept_id = ? AND name LIKE (?)",
        concept_id,"%#{params[:search_string]}%"]).group("name").select("name").map{|c|c.name}
  ...
  def last_appointment_date(patient_id, date=Date.today)
    ...
    enc = Encounter.where(["patient_id=? and encounter_type=#{encounter_type_id} and Date(encounter_datetime) <=DATE(?)",
        patient_id, date.to_date]).order("encounter_datetime desc").first
  ...
  def patient_present?(patient_id, date=Date.today)
    ...
    patient_present = encounter.observations.where(["_concept_id =?", concept_id]).last.to_s unless encounter.nil?
  ...
  def guardian_present?(patient_id, date=Date.today)
    ...
    guardian_present=encounter.observations.where(["concept_id =?", concept_id]).last.to_s unless encounter.nil?
  ...
  def outcome(patient_id, on_date=Date.today)
    state = PatientState.joins("INNER JOIN patient_program p ON p.patient_program_id = patient_state.patient_program_id").where(
      ["patient_state.voided = 0 AND p.voided = 0 AND p.patient_id = #{patient_id} AND DATE(start_date) <= DATE('#{on_date}')"]
  ...
  def all_appointment_dates(start_date, end_date = nil)
    ...
    appointments = Patient.joins("INNER JOIN obs ON patient.patient_id = obs.person_id").where(
      ["DATE(obs.value_datetime) >= ? AND DATE(obs.value_datetime) <= ? AND obs.concept_id = ? AND obs.voided = 0",
        start_date.to_date, end_date.to_date, appointment_date_concept_id]).group("obs.person_id")
