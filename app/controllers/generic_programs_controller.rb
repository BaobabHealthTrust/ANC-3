class GenericProgramsController < ApplicationController
  ...
  def new
    ...
    program_names = PatientProgram.joins("INNER JOIN location l ON l.location_id = patient_program.location_id INNER JOIN program p ON p.program_id = patient_program.program_id").where(
      ["voided = 0 AND patient_id = ? AND date_completed IS NULL",params[:patient_id]]
    ).select("p.name program_name ,l.name location_name,patient_program.date_completed date_completed").map{|pat_program|
  ...
  def create
    active_programs = PatientProgram.where(["voided = 0 AND patient_id = ? AND location_id = ? AND program_id = ?", @patient.id,params[:location_id],params[:program_id]])
    invalid_date = false
  ...
  def states
    if params[:show_non_terminal_states_only].to_s == true.to_s
      @states = ProgramWorkflowState.includes(:concept).where(['program_workflow_id = ? AND terminal = 0', params[:workflow]])
    else
