class GenericDispensationsController < ApplicationController
  def new
    ...
    @prescriptions = Order.joins("INNER JOIN encounter e USING (encounter_id)").where(["encounter_type = ? AND
      e.patient_id = ? AND DATE(encounter_datetime) = ?", type.id,@patient.id,session_date])
  ...
  def create
    ...
    if obs.save
      ...
      @patient.patient_programs.where(["program_id =?", Program.find_by_name("DIABETES PROGRAM").program_id]).last.transition(
      ...
      if !@tb_programs.blank?
        @patient.patient_programs.where(["program_id =?", Program.find_by_name("TB PROGRAM").program_id]).last.transition(
  ...
  def quantities 
    ...
    amounts = []
    Observation.question("AMOUNT DISPENSED").where({:value_drug => drug.drug_id}).group("value_drug, value_numeric").order("count(*)").limit(10).each do |obs|
...
EOF
    ...
    if (Observation.where(["person_id = ? AND encounter_id = ? AND concept_id = ?",
            patient.id,encounter.id,ConceptName.find_by_name('ARV REGIMENS RECEIVED ABSTRACTED CONSTRUCT').concept_id]).first).blank?
