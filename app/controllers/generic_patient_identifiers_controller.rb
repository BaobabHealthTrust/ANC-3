class GenericPatientIdentifiersController < ApplicationController
  ...
  def edit
    if request.post? && params[:type] && params[:identifier]
      ...
      if patient_identifier_exists(params[:identifier],params[:type], number_prefix)
        ...
        current_identifiers = patient.patient_identifiers.where(["identifier_type =?", identifier_type.id])
