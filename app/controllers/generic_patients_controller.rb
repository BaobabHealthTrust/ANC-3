class GenericPatientsController < ApplicationController
  ...
  def opdshow
    ...
    @encounters = Encounter.joins("INNER JOIN encounter_type ON encounter_type_id = encounter_type").where(["patient_id = ? AND encounter_type IN (?) AND DATE(encounter_datetime) = ?",
        params[:id],encounter_types,session_date]).group("encounter_type").select("encounter_id , name encounter_type_name, count(*) c").collect do |rec|
  ...
  def treatment
    ...
    session_date = session[:datetime].to_date rescue Date.today
    @prescriptions = Order.joins("INNER JOIN encounter e USING (encounter_id)").where(["encounter_type = ? AND e.patient_id = ? AND DATE(encounter_datetime) = ?",
  ...
  def history_treatment
    ...
    type = EncounterType.find_by_name('TREATMENT')
    session_date = session[:datetime].to_date rescue Date.today
    @prescriptions = Order.joins("INNER JOIN encounter e USING (encounter_id)").where(["encounter_type = ? AND e.patient_id = ?",type.id,@patient.id])
...
=end
  ...
  def visit_history
    ...
    @encounters = Encounter.where([" patient_id = ? AND encounter_datetime >= ? AND encounter_datetime <= ?", @patient.id, start_date, end_date])
  ...
  def get_previous_encounters(patient_id)
    previous_encounters = Encounter.includes([:observations]).where(["encounter.voided = ? and patient_id = ?", 0, patient_id])
  ...
  def treatment_dashboard
    ...
    type = EncounterType.find_by_name('TREATMENT')
    session_date = session[:datetime].to_date rescue Date.today
    Order.joins("INNER JOIN encounter e USING (encounter_id)").where(["encounter_type = ? AND e.patient_id = ? AND DATE(encounter_datetime) = ?",
  ...
  def alerts(patient, session_date = Date.today) 
    ...
    @show_change_app_date = Observation.joins("INNER JOIN encounter ON obs.encounter_id = encounter.encounter_id").where(
      ["concept_id = ? AND encounter_type = ? AND patient_id = ?
    AND encounter_datetime >= ? AND encounter_datetime <= ?",
        ...
        session_date.strftime("%Y-%m-%d 23:59:59")]).order("encounter_datetime DESC,encounter.date_created DESC").first != nil
    ...
    next_appt = Observation.joins("INNER JOIN encounter ON obs.encounter_id = encounter.encounter_id").where(
      ...
      ]).order("encounter_datetime DESC,encounter.date_created DESC").first.value_datetime.strftime("%a %d %B %Y") rescue nil
    ...
    missed_appt = patient.encounters.where(["encounter_type =?", type.id]).where(["NOT (DATE_FORMAT(encounter_datetime, '%Y-%m-%d') IN (?)) AND encounter_datetime < NOW()",
        encounter_dates]).order("encounter_datetime").last.observations.last.to_s rescue nil
    ...
    observations = Observation.joins("INNER JOIN encounter e USING(encounter_id)").where(
      ["concept_id = ? AND encounter_type = ? AND patient_id = ? AND
    ...
    patient.encounters.where(["encounter_type =?", type.id]).order("encounter_datetime").last.observations.each do | obs |
    ...
    patient_hiv_program = PatientProgram.where(["voided = 0 AND patient_id = ? AND program_id = ? AND location_id = ?", patient.id , program_id, location_id])
    ...
    unless encounter_date.blank?
      ...
      if smear_positive_patient == true
        ...
        if date_diff > 60 and date_diff < 110
          results = Encounter.includes([:observations]).where(["encounter_type = ? and " \
                "patient_id = ? AND encounter_datetime BETWEEN ? AND ?",
              EncounterType.find_by_name("LAB RESULTS").id,
              patient_id, (encounter_date + 60).to_s, (encounter_date + 110).to_s]).last rescue ''
          ...
          results = Encounter.includes([:observations]).where(["encounter_type = ? and " \
                "patient_id = ? AND encounter_datetime BETWEEN ? AND ?",
              EncounterType.find_by_name("LAB RESULTS").id,
              patient_id, (encounter_date + 111).to_s, (encounter_date + 140).to_s]).last rescue ''
...
=end
  ...
  def mastercard_demographics(patient_obj)
    ...
    regimens.map do | regimen_type , ids |
      encounter = Encounter.joins("INNER JOIN obs ON encounter.encounter_id = obs.encounter_id").where(
        ["encounter_type=? AND encounter.patient_id = ? AND concept_id = ? AND encounter.voided = 0",
          encounter_type , patient_obj.id , amount_dispensed_concept_id ]).order("encounter_datetime").first
    ...
    art_initial = Encounter.where(["encounter_type = ? and patient_id = ?",
        EncounterType.find_by_name("ART_INITIAL").id,patient_obj.id]).last
  ...
  def visits(patient_obj,encounter_date = nil)
    ...
    program_id = Program.find_by_name('HIV PROGRAM').id
    if encounter_date.blank?
      patient_states = PatientState.joins("INNER JOIN patient_program p ON p.patient_program_id = patient_state.patient_program_id").where(
        ["patient_state.voided = 0 AND p.voided = 0 AND p.program_id = ? AND p.patient_id = ?", program_id,patient_obj.patient_id]).order("patient_state_id ASC")
    else
      patient_states = PatientState.joins("INNER JOIN patient_program p ON p.patient_program_id = patient_state.patient_program_id").where(
        ["patient_state.voided = 0 AND p.voided = 0 AND p.program_id = ? AND start_date = ? AND p.patient_id =?",
...
=end
    ...
    unless encounter_date.blank? 
      ...
      if outcome.blank?
        state = PatientState.joins("INNER JOIN patient_program p ON p.patient_program_id = patient_state.patient_program_id").where(
          ["patient_state.voided = 0 AND p.voided = 0 AND p.program_id = ? AND p.patient_id = ?",
            program_id,patient_obj.patient_id]).order("date_enrolled DESC,start_date DESC").first
  ...
  def save_mastercard_attribute(params)
    ...
    when "occupation"
      ...
      exists_person_attribute = PersonAttribute.where(["person_id = ? AND person_attribute_type_id = ?", patient.person.id, occupation_attribute.person_attribute_type_id]).first rescue nil
  ...
  def set_new_patient_filing_number(patient)
    ActiveRecord::Base.transaction do
      ...
      if (next_filing_number[5..-1].to_i >= global_property_value.to_i)
        ...
        all_filing_numbers = PatientIdentifier.where(["identifier_type = ?", filing_number_identifier_type.id]).group("patient_id")
        ...
        if patient_to_be_archived.blank?
          patient_to_be_archived = PatientIdentifier.where(["identifier_type = ?",
              filing_number_identifier_type.id]).group("patient_id").order("identifier DESC").last.patient rescue nil
        end
      end
  ...
  def get_recent_screen_complications
    ...
    @vitals = Encounter.where(["patient_id= ? AND encounter_datetime < ? ",
        @patient.patient_id, Time.now.to_date]).order("obs_datetime DESC").limit(50)
  ...
  def patient_medical_history
    ...
    @encounter_type_ids = EncounterType.where(["name IN (?)", encounters_list]).each{|e| e.encounter_type_id}
  ...
  def generate_booking
    ...
    if(@type)
      @enc = Encounter.where(["voided = 0 AND encounter_type = ?", @type])
  ...
  def remove_booking
    if(params[:patient_id])
      ...
      if(@type)
        @enc = @patient.encounters.joins(:observations).where(['encounter_type = ?', @type])
  ...
  def complications
    ...
    @creatinine_obs = @patient.person.observations.joins(:encounter).where(['encounter_type = ? AND concept_id = ?',
        diabetes_test_id, creatinine_id]).order("obs_datetime DESC")

    # Urine Protein
    urine_protein_id = Concept.find_by_name('URINE PROTEIN').id
    @urine_protein_obs = @patient.person.observations.joins(:encounter).where(['encounter_type = ? AND concept_id = ?',
        diabetes_test_id, urine_protein_id]).order("obs_datetime DESC")

    # Foot Check
    @foot_check_encounters = @patient.encounters.joins(:observations).where(['concept_id IN (?)',
        ConceptName.where(["name IN (?)", ['RIGHT FOOT/LEG','LEFT FOOT/LEG', 'LEFT HAND/ARM', 'RIGHT HAND/ARM']
          ]).map(&:concept_id)]).order("obs_datetime DESC").uniq
    ...
    @foot_check_encounters.each{|e|
      value = @patient.person.observations.joins(:encounter).where(['encounter_type = ? AND encounter.encounter_id IN (?)',
          diabetes_test_id, e.encounter_id]).order("obs_datetime DESC")
    ...
    @visual_acuity_encounters = @patient.encounters.joins(:observations).where(['concept_id IN (?)',
        ConceptName.where(["name IN (?)", ['LEFT EYE VISUAL ACUITY','RIGHT EYE VISUAL ACUITY']
          ]).map(&:concept_id)]).order("obs_datetime DESC").uniq
    ...
    @visual_acuity_encounters.each{|e|
      @visual_acuity_obs[e.encounter_id] = @patient.person.observations.joins(:encounter).where(['encounter_type = ? AND encounter.encounter_id = ?',
          diabetes_test_id, e.encounter_id]).order("obs_datetime DESC")
    }


    # Fundoscopy
    @fundoscopy_encounters = @patient.encounters.joins(:observations).where(['concept_id IN (?)',
        ConceptName.where(["name IN (?)", ['LEFT EYE FUNDOSCOPY','RIGHT EYE FUNDOSCOPY']
          ]).map(&:concept_id)]).order("obs_datetime DESC").uniq
    ...
    @fundoscopy_encounters.each{|e|
      @fundoscopy_obs[e.encounter_id] = @patient.person.observations.joins(:encounter).where(['encounter_type = ? AND encounter.encounter_id IN (?)',
          diabetes_test_id, e.encounter_id]).order("obs_datetime DESC")
    }
    
    # Urea
    urea_id = Concept.find_by_name('UREA').id
    @urea_obs = @patient.person.observations.joins(:encounter).where(['encounter_type = ? AND concept_id = ?',
        diabetes_test_id, urea_id]).order("obs_datetime DESC")


    # Macrovascular
    macrovascular_id = Concept.find_by_name('MACROVASCULAR').id
    @macrovascular_obs = @patient.person.observations.joins(:encounter).where(['encounter_type = ? AND concept_id = ?',
        diabetes_test_id, macrovascular_id]).order("obs_datetime DESC")
