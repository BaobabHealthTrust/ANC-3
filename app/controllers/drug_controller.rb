class DrugController < ApplicationController
  def management
    ...
    new_deliveries = Pharmacy.active.where(["pharmacy_encounter_type=?",encounter_type]).order("encounter_date DESC,date_created DESC")
    ...
    current_stock.each{|delivery_id , delivery|
      start_date = Pharmacy.active.where(["drug_id =?", delivery.drug_id]).order("encounter_date").first.encounter_date.to_date rescue nil
  ...
  def name
    @names = Drug.where(["name LIKE ?","%" + params[:search_string] + "%"]).collect{|drug| drug.name}
    render :text => "<li>" + @names.map{|n| n } .join("</li><li>") + "</li>"
  end

  def delivery
    @drugs = Drug.all.map{|d|d.name}.compact.sort rescue []
  end
  ...
  def stock_report
    ...
    @stock = {}
    current_stock.each{|delivery_id , delivery|
      first_date = Pharmacy.active.where(["drug_id =?", delivery.drug_id]).order("encounter_date").first.encounter_date.to_date rescue nil
  ...
  def add_drug_set
    ...
    @drugs = [["", ""]] + Drug.where(["drug_id NOT IN (?)", already_selected]).collect{|drug| [drug.name, drug.id]}
