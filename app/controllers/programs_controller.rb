class ProgramsController < ApplicationController
  ...
  def new
    ...
    program_names = PatientProgram.where(["voided = 0 AND patient_id = ? AND location_id = ?",
  ...
  def workflows
    @workflows = ProgramWorkflow.joins(:concept).where(['program_id = ?', params[:program]])
    @names = @workflows.map{|workflow| "<li value='#{workflow.id}'>#{workflow.concept.fullname}</li>" }
    render plain: @names.join('')
  end
  
  def states
    @states = ProgramWorkflowState.joins(:concept).where(['program_workflow_id = ?', params[:workflow]])
  ...
  def update
    if request.method == :post
      ...
      @patient_program_id = patient_program.patient_program_id
      program_workflow = ProgramWorkflow.joins(:concept).where(['program_id = ?', patient_program.program_id])
      @program_workflow_id = program_workflow.first.program_workflow_id
      @states = ProgramWorkflowState.joins(:concept).where(['program_workflow_id = ?', @program_workflow_id])
