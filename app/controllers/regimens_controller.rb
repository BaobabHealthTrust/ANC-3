class RegimensController < ApplicationController
  ...
  def dosing
    @patient = Patient.find(params[:patient_id] || session[:patient_id]) rescue nil
    @criteria = Regimen.criteria(@patient.current_weight).includes(:regimen_drug_orders).where({:concept_id => params[:id]})
  ...
  def durations
    @regimen = Regimen.includes(:regimen_drug_orders).find_by_concept_id(params[:id])
    ...
    amounts = []
    orders = DrugOrder.joins("LEFT JOIN orders ON orders.order_id = drug_order.order_id AND orders.voided = 0").where({:drug_inventory_id => @drug_id}
