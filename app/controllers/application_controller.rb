...
=end
    ...
    if @anc_patient.hiv_status.downcase == "positive" and
      ...
      if !art_link.nil? && !anc_link.nil? # && foreign_links.include?(pos)
        ...
        if !File.exists?("#{Rails.root.to_s}/config/dde_connection.yml")
          @external_encounters = Bart2Connection::PatientIdentifier.search_by_identifier(@anc_patient.national_id).patient.encounters.where(
            ["encounter_datetime = ?", (session[:datetime] || Time.now()).to_date.strftime("%Y-%m-%d")]).collect{|e| e.type.name}
        else
          @external_encounters = Bart2Connection::PatientIdentifier.search_or_create(@anc_patient.national_id).patient.encounters.where(
            ["encounter_datetime = ?", (session[:datetime] || Time.now()).to_date.strftime("%Y-%m-%d")]).collect{|e| e.type.name}
        end
    ...
    sorted_tasks.each do |pos, tsk|
      ...
      when "TODAY"
        ...
        if !tasks[tsk][6].nil? && checked_already == false   # Check for drug concept if available
          available = patient.orders.where(["concept_id = ? AND start_date = ?",
        ...
        if checked_already == false
          available = Encounter.joins([:observations]).where(["patient_id = ? AND encounter_type = ? AND DATE(encounter_datetime) = ?",
      ...
      when "RECENT"
        ...
        if !tasks[tsk][3].nil? && checked_already == false    # Check for presence of specific concept_id
          available = Encounter.joins([:observations]).where(["patient_id = ? AND encounter_type = ? AND obs.concept_id = ? " +
        ...
        if !tasks[tsk][4].nil? && checked_already == false   # Check for concept exclusions from encounter_type group
          available = Encounter.joins([:observations]).where(["patient_id = ? AND encounter_type = ? AND NOT obs.concept_id = ? " +
                "AND (DATE(encounter_datetime) >= ? AND DATE(encounter_datetime) <= ?)",
      ...
      when "EXISTS"
        ...
        if !tasks[tsk][6].nil? && checked_already == false   # Check for drug concept if available
          available = patient.orders.where(["concept_id = ?",
              tasks[tsk][6]]) rescue []
  ...
  def main_next_task(location, patient, session_date = Date.today)
    ...
    all_tasks = Task.order('sort_weight ASC')
    ...
    all_tasks.each do |task|
      ...
      if task.has_obs_concept_id.present?
        ...
        if (task.has_obs_scope == 'RECENT')
          o = patient.person.observations.recent(1).where(['encounter_id IN (?) AND concept_id =? AND DATE(obs_datetime)=?',
              todays_encounters.map(&:encounter_id), task.has_obs_concept_id,session_date]).first
      ...
      if task.has_obs_value_drug && task.has_obs_scope == 'TODAY'
        obs = patient.orders.where(["concept_id = ? AND start_date = ?",
            task.has_obs_value_drug, session_date]).first
      ...
      if (task.has_obs_scope == 'EXISTS')
        obs = patient.person.observations.where(['encounter_id IN (?)',
            active_encounters.where(["encounter_type = ?",
                EncounterType.find_by_name(task.encounter_type).id]).map(&:encounter_id)]).first
      ...
      if task.has_order_type_id.present?
        skip = true unless Order.unfinished.where({:order_type_id => task.has_order_type_id}).first.present?
      end
