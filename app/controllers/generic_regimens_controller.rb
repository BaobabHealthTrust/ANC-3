class GenericRegimensController < ApplicationController
	...
	def new
		...
		@tb_encounter = Encounter.includes([:observations]).where(["patient_id = ? AND encounter_type = ?",
        @patient.id, EncounterType.find_by_name("TB visit").id]).order("encounter_datetime DESC,date_created DESC").first rescue nil
    ...
    (@current_regimens_for_programs || {}).each do |patient_program_id , regimen_id|
      ...
      @regimen_formulations = formulation(@patient,regimen_id)
		...
		@current_hiv_program_state = PatientProgram.joins(:location).where(["program_id = ? AND location.location_id = ? AND date_completed IS NULL",
        Program.find_by_concept_id(Concept.find_by_name('HIV PROGRAM').id).id, Location.current_health_center
      ]).first.patient_states.current.first.program_workflow_state.concept.fullname rescue ''


		session_date = session[:datetime].to_date rescue Date.today

		pre_art_visit = Encounter.where(["DATE(encounter_datetime) = ? AND patient_id = ? AND encounter_type = ?",
		...
		art_visit = Encounter.where(["DATE(encounter_datetime) = ? AND patient_id = ? AND encounter_type = ?",
        session_date.to_date, @patient.id, EncounterType.find_by_name('ART VISIT').id]).order("encounter_datetime DESC,date_created DESC").first
		@art_visit = false
		...
		treatment_obs = Encounter.where(["DATE(encounter_datetime) = ? AND patient_id = ? AND encounter_type = ?",
		    session_date, @patient.id, EncounterType.find_by_name('TREATMENT').id]).order("encounter_datetime DESC,date_created DESC").first.observations rescue []
		...
		tb_visit_obs = Encounter.where(["DATE(encounter_datetime) = ? AND patient_id = ? AND encounter_type = ?",
		    session_date, @patient.id, EncounterType.find_by_name('TB VISIT').id]).order("encounter_datetime DESC,date_created DESC").first.observations rescue []
        ...
        @patient.id, EncounterType.where(["name IN (?)",["ART VISIT", "TB VISIT"]]).select("encounter_type_id"),session_date.to_date]).order("encounter_datetime DESC,date_created DESC").first.observations rescue []
		...
		art_visit_obs = Encounter.where(["patient_id = ? AND encounter_type IN (?) AND DATE(encounter_datetime) = ?",
        @patient.id, EncounterType.where(["name IN (?)",["ART VISIT"]]).select("encounter_type_id"),session_date.to_date]).order("encounter_datetime DESC,date_created DESC").first.observations rescue []
	...
	def create
		...
		reduced = false
		orders = RegimenDrugOrder.where({:regimen_id => params[:regimen]})
		ActiveRecord::Base.transaction do
			# Need to write an obs for the regimen they are on, note that this is ARV
			# Specific at the moment and will likely need to have some kind of lookup
			# or be made generic
	...
	def dosing
		@patient = Patient.find(params[:patient_id] || session[:patient_id]) rescue nil
		@criteria = Regimen.criteria(PatientService.get_patient_attribute_value(@patient, "current_weight")).includes(:regimen_drug_orders).where({:concept_id => params[:id]})
	...
	def durations
		@regimen = Regimen.find_by_concept_id(params[:id])
    ...
    ).group("drug_inventory_id, DATEDIFF(orders.auto_expire_date, orders.start_date)").order("count(*)").select("DATEDIFF(orders.auto_expire_date, orders.start_date) as duration_days").limit(10)
		orders.each {|order|
