module PatientService
  ...
  def self.count_by_type_for_date(date)
    ...
    todays_encounters = Encounter.includes("type").where(["encounter_datetime BETWEEN TIMESTAMP (?) AND TIMESTAMP (?)",
        date.to_date.strftime('%Y-%m-%d 00:00:00'),
        date.to_date.strftime('%Y-%m-%d 23:59:59')
      ])
  ...
  def self.checks_if_labs_results_are_avalable_to_be_shown(patient , session_date , task)
    lab_result = Encounter.where([" encounter_datetime <= TIMESTAMP(?) AND patient_id = ? AND encounter_type = ?",
        session_date.to_date.strftime('%Y-%m-%d 23:59:59'),
        patient.id,
        EncounterType.find_by_name('LAB RESULTS').id]).order("encounter_datetime DESC").first

    give_lab_results = Encounter.where(["encounter_datetime >= TIMESTAMP(?)
                                AND patient_id = ? AND encounter_type = ?",
        lab_result.encounter_datetime.to_date.strftime('%Y-%m-%d 00:00:00') , patient.id,
        EncounterType.find_by_name('GIVE LAB RESULTS').id]).order("encounter_datetime DESC").first rescue nil
  ...
  def self.recent_sputum_submissions(patient_id)
    ...
    Observation.where(["person_id = ? AND concept_id = ? AND (value_coded in (?) OR value_text in (?))",patient_id, 
        ConceptName.find_by_name('Sputum submission').concept_id, sputum_concept_ids, sputum_concept_names]).order("obs_datetime desc").limit(3) rescue []
  end
  ...
  def self.recent_sputum_results(patient_id)
    ...
    sputum_concept_ids = ConceptName.where(["name IN (?)", sputum_concept_names]).map(&:concept_id)
    obs = Observation.where(["person_id = ? AND concept_id IN (?)", patient_id, sputum_concept_ids]).order("obs_datetime desc").limit(3)
  end

  def self.sputum_orders_without_submission(patient_id)
    self.recent_sputum_orders(patient_id).collect{|order| order unless Observation.where(["person_id = ? AND concept_id = ?", patient_id, Concept.find_by_name("Sputum submission")]).map{|o| o.accession_number}.include?(order.accession_number)}.compact #rescue []
  end

  def self.recent_sputum_orders(patient_id)
    sputum_concept_names = ["AAFB(1st)", "AAFB(2nd)", "AAFB(3rd)", "Culture(1st)", "Culture(2nd)"]
    sputum_concept_ids = ConceptName.where(["name IN (?)", sputum_concept_names]).map(&:concept_id)
    Observation.where(["person_id = ? AND concept_id = ? AND (value_coded in (?) OR value_text in (?))", patient_id, ConceptName.find_by_name('Tests ordered').concept_id,
        sputum_concept_ids, sputum_concept_names]).order("obs_datetime desc").limit(3)
  end

  def self.hiv_test_date(patient_id)
    test_date = Observation.where(["person_id = ? AND concept_id = ?", patient_id,
        ConceptName.find_by_name("HIV test date").concept_id]).last.value_datetime rescue nil
    return test_date
  end

  def self.months_since_last_hiv_test(patient_id)
    #this can be done better
    session_date = Observation.where(["person_id = ? AND concept_id = ?", patient_id,
        ConceptName.find_by_name("HIV test date").concept_id]).last.obs_datetime rescue Date.today
  ...
  def self.patient_hiv_status(patient)
    status = Concept.find(Observation.where(["value_coded IS NOT NULL AND person_id = ? AND concept_id = ?", patient.id,
          ConceptName.find_by_name("HIV STATUS").concept_id]).order("obs_datetime DESC,date_created DESC").first.value_coded).fullname rescue "UNKNOWN"
  ...
  def self.patient_tb_status(patient)
    Concept.find(Observation.where(["person_id = ? AND concept_id = ? AND value_coded IS NOT NULL",patient.id,
          ConceptName.find_by_name("TB STATUS").concept_id]).order("obs_datetime DESC,date_created DESC").first.value_coded).fullname rescue "UNKNOWN"
  end
  ...
  def self.patient_appointment_dates(patient, start_date, end_date = nil)
    ...
    appointments = Observation.where(["obs.value_datetime BETWEEN TIMESTAMP(?) AND TIMESTAMP(?) AND " +
  ...
  def self.get_patient_identifier(patient, identifier_type)
    ...
    patient_identifier = PatientIdentifier.where(["patient_id = ? and identifier_type = ?", patient.id,
        patient_identifier_type_id]).select("identifier").order("date_created DESC").first.identifier rescue nil
...
EOF
  ...
  def self.art_patient?(patient)
    ...
    enrolled = PatientProgram.where(["program_id = ? AND patient_id = ?",program_id,patient.id]).first.blank?
...
EOF
  ...
  def self.prescribe_arv_this_visit(patient, date = Date.today)
    ...
    refer_patient = Encounter.joins("INNER JOIN obs USING (encounter_id)").where(["encounter_type = ? AND concept_id = ? AND person_id = ? AND value_coded = ? AND obs_datetime BETWEEN ? AND ?",
  ...
  def self.drug_given_before(patient, date = Date.today)
    ...
    Order.joins("INNER JOIN obs ON obs.order_id = orders.order_id").where(["obs.person_id = ? AND obs.concept_id = ?
  ...
  def self.drugs_given_on(patient, date = Date.today)
    ...
    latest_encounter_date = Encounter.where(["patient_id = ? AND encounter_datetime >= ?
        AND encounter_datetime <=? AND encounter_type IN(?)",
        patient.id,date.strftime('%Y-%m-%d 00:00:00'),
        date.strftime('%Y-%m-%d 23:59:59'),encounter_type_ids]).order("encounter_datetime DESC").first.encounter_datetime rescue nil
	...
	def self.next_filing_number_to_be_archived(current_patient , next_filing_number)
		ActiveRecord::Base.transaction do
            ...
            PatientIdentifierType.find_by_name("Filing Number").id]).group("patient_id")
              ...
              PatientIdentifierType.find_by_name("Filing Number").id]).order("identifier DESC").group("patient_id").last.patient rescue nil
				end
			end
            ...
            patient_to_be_archived.id,PatientIdentifierType.find_by_name("Filing Number").id]).last
...
=end
  ...
  def self.person_search(params)
    ...
    people = Person.joins([{:names => [:person_name_code]}]).where([
    ...
    if people.length < 15
      ...
      people_like = Person.joins([{:names => [:person_name_code]}]).where([
          "person_name_code.given_name_code LIKE ? AND \
     person_name_code.family_name_code LIKE ? AND person.person_id NOT IN (?)",
          (given_name || '').soundex,
          (family_name || '').soundex,
          matching_people
        ]).limit(15).order("person_name.given_name ASC, person_name_code.family_name_code ASC")
...
=end
  ...
  def self.next_lab_encounter(patient , encounter = nil , session_date = Date.today)
    if encounter.blank?
      ...
      lab_order = Encounter.where(["patient_id = ? AND encounter_type = ?",patient.id,type]).order("encounter_datetime DESC,date_created DESC").first
    ...
    when 'LAB ORDERS'
      ...
      sputum_sub = Encounter.joins("INNER JOIN obs USING(encounter_id)").where(["obs.accession_number IN (?) AND patient_id = ? AND encounter_type = ?",
          encounter.observations.map{|r|r.accession_number}.compact,encounter.patient_id,type]).first

      return type if sputum_sub.blank?
      return sputum_sub
    when 'SPUTUM SUBMISSION'
      type = EncounterType.find_by_name('LAB RESULTS').id
      lab_results = Encounter.joins("INNER JOIN obs USING(encounter_id)").where(["obs.accession_number IN (?) AND patient_id = ? AND encounter_type = ?",
          encounter.observations.map{|r|r.accession_number}.compact,encounter.patient_id,type]).first

      type = EncounterType.find_by_name('LAB ORDERS').id
      lab_order = Encounter.joins("INNER JOIN obs USING(encounter_id)").where(["obs.accession_number IN (?) AND patient_id = ? AND encounter_type = ?",
          encounter.observations.map{|r|r.accession_number}.compact,encounter.patient_id,type]).first
  ...
  def self.need_art_enrollment(task,patient,location,session_date,user_selected_activities,reason_for_art)
    ...
    enrolled_in_hiv_program = Concept.find(Observation.where(["person_id = ? AND concept_id = ?",patient.id,
          ConceptName.find_by_name("Patient enrolled in IMB HIV program").concept_id]).order("obs_datetime DESC,date_created DESC").first.value_coded).concept_names.map{|c|c.name}[0].upcase rescue nil
    ...
    art_initial = Encounter.where(["patient_id = ? AND encounter_type = ?",
        patient.id,EncounterType.find_by_name('ART_INITIAL').id]).order("encounter_datetime DESC,date_created DESC").limit(1).first
    ...
    treatment_encounter = Encounter.joins("INNER JOIN obs USING(encounter_id)").where(["patient_id = ? AND encounter_type = ? AND concept_id = ?",
        patient.id,EncounterType.find_by_name('TREATMENT').id,ConceptName.find_by_name('ARV regimen type').concept_id]).order("encounter_datetime DESC").first
